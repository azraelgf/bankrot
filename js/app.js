(() => {
    ...
    (() => {
        ...
        if (document.querySelector("[data-datepicker]")) document.querySelectorAll("[data-datepicker]").forEach((function(element) {
            datepicker_min(element, {
        ...
        function documentActions(e) {
            const targetElement = e.target;
            if (targetElement.closest(".search-btn")) document.querySelector(".header__search-wrapper").classList.toggle("_active");
        }
        const formImage = document.querySelectorAll(".file__input");
        formImage.forEach((el => {
            el.addEventListener("change", (() => {
                const activeInput = document.querySelector("._active-input");
                activeInput ? activeInput.classList.remove("_active-input") : null;
                el.parentNode.parentElement.parentElement.classList.add("_active-input");
                const formText = document.querySelector("._active-input .input-wrapper__text"), urlImage = el.value;
                console.log(urlImage);
                uploadFile(el.files[0], formText, urlImage);
            }));
        }));
        function uploadFile(file, formText, urlImage) {
            ...
            formText.setAttribute("value", `${imageUrl}`);
        }
        window["FLS"] = false;
        menuInit();
        formFieldsInit({
